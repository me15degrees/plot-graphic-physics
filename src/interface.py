...
text_description = """  Nesse trabalho foi instruído para que aplicasse a parte conceitual da
    ...
    da posição por meio da aceleração de particula."""
...
tabview = ctk.CTkTabview(
    window, width=1920, height=1080, fg_color="grey", bg_color="#242424"
)
tabview.pack(padx=10, pady=10)
tabview.add("Gráficos")
tabview.add("Descrição")

frame_descr = ctk.CTkFrame(
    tabview.tab("Descrição"), width=700, height=700, fg_color="#242424"
)
frame_descr.pack(side=ctk.LEFT, pady=20, padx=20)

frame_inter = ctk.CTkFrame(
    tabview.tab("Descrição"), width=700, height=700, fg_color="#242424"
)
...
frame_btn = ctk.CTkFrame(
    tabview.tab("Gráficos"), width=1920, height=1080, fg_color="grey"
)
frame_btn.pack(side=ctk.LEFT, pady=20, padx=20)

frame_graph = ctk.CTkFrame(tabview.tab("Gráficos"), fg_color="white")
frame_graph.pack(padx=10, pady=5)

instruction = ctk.CTkLabel(
    tabview.tab("Gráficos"), text="Selecione uma opção de gráfico abaixo:"
)
instruction.pack(padx=10, pady=0)

btn_1 = ctk.CTkButton(frame_btn, width=150, text="Gráfico 1 - Posição", command=selec)
btn_1.pack(pady=30, padx=25)

btn_2 = ctk.CTkButton(
    frame_btn, width=150, text="Gráfico 2 - Velocidade", command=selectwo
)
btn_2.pack(pady=30, padx=25)

btn_3 = ctk.CTkButton(
    frame_btn, width=150, text="Gráfico 3 - Aceleração", command=selecttree
)
